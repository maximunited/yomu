...
console.log(`   ${pageHeaderExists ? '✅' : '❌'} src/components/PageHeader.tsx`);

// Test 3: Check if translation keys exist
console.log('\n✅ Checking translation keys:');
const translationKeys = [
  'about', 'terms', 'privacy', 'contact', 'back',
  'contactInformation', 'sendMessage', 'phone', 'address',
  'workingHours', 'chooseSubject', 'technicalSupport',
  'improvementSuggestion', 'bugReport', 'generalQuestion',
  'writeYourMessageHere', 'enterYourFullName', 'yourEmail',
  'allRightsReserved'
];

const translationsFile = fs.readFileSync('src/lib/translations.ts', 'utf8');
const missingKeys = translationKeys.filter(key => !translationsFile.includes(key));
...
pages.forEach(page => {
  if (fs.existsSync(page)) {
    ...
    const hasDarkModeClasses = content.includes('isDarkMode') && content.includes('bg-gray-900');
    console.log(`   ${page}: ${hasDarkModeClasses ? '✅' : '❌'} Dark mode classes`);
  }
});
...
pages.forEach(page => {
  if (fs.existsSync(page)) {
    const content = fs.readFileSync(page, 'utf8');
    const hasTranslationUsage = content.includes('t(') && content.includes('useLanguage');
    console.log(`   ${page}: ${hasTranslationUsage ? '✅' : '❌'} Translation usage`);
  }
});
