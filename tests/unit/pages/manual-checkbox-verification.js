...
if (fs.existsSync(signinPagePath)) {
  ...
  const hasKeepSignedInState = signinContent.includes(
    "keepSignedIn, setKeepSignedIn",
  );

  console.log(
    `✅ Save Email state: ${hasSaveEmailState ? "Found" : "Missing"}`,
  );
  console.log(
    `✅ Keep Signed In state: ${hasKeepSignedInState ? "Found" : "Missing"}`,
  );
  ...
  console.log(
    `✅ Save Email checkbox: ${hasSaveEmailCheckbox ? "Found" : "Missing"}`,
  );
  console.log(
    `✅ Keep Signed In checkbox: ${
      hasKeepSignedInCheckbox ? "Found" : "Missing"
    }`,
  );
  ...
  console.log(
    `✅ Save Email label: ${hasSaveEmailLabel ? "Found" : "Missing"}`,
  );
  console.log(
    `✅ Keep Signed In label: ${hasKeepSignedInLabel ? "Found" : "Missing"}`,
  );

  // Check for proper styling classes
  const hasProperStyling =
    signinContent.includes("space-y-4") &&
    signinContent.includes("flex items-start space-x-3");

  console.log(
    `✅ Proper spacing classes: ${hasProperStyling ? "Found" : "Missing"}`,
  );

  // Check for translation keys
  const hasTranslationKeys =
    signinContent.includes("t('saveEmail')") &&
    signinContent.includes("t('keepMeSignedIn')");

  console.log(
    `✅ Translation keys: ${hasTranslationKeys ? "Found" : "Missing"}`,
  );
} else {
...
if (fs.existsSync(translationsPath)) {
  ...
  const hasKeepSignedInTranslation =
    translationsContent.includes("keepMeSignedIn:");

  console.log(
    `✅ Save Email translation: ${
      hasSaveEmailTranslation ? "Found" : "Missing"
    }`,
  );
  console.log(
    `✅ Keep Signed In translation: ${
      hasKeepSignedInTranslation ? "Found" : "Missing"
    }`,
  );
} else {
...
console.log(
  '- Each checkbox should be in a flex container with "items-start space-x-3"',
);
console.log(
  '- Checkboxes should have purple styling: "text-purple-600 focus:ring-purple-500"',
);
console.log("- Labels should have proper text colors for light/dark mode");
console.log(
  '- Checkboxes should have proper IDs and labels with "htmlFor" attributes',
);
