...
describe('/api/admin/benefits/[id]', () => {
  ...
  it('PATCH updates isActive', async () => {
    ;(prismaClient.prisma.benefit.update as jest.Mock).mockResolvedValueOnce({ id: 'b1', isActive: false })
    const req = new Request('http://localhost', { method: 'PATCH', body: JSON.stringify({ isActive: false }) } as any) as any
  ...
  it('DELETE removes notifications and benefit', async () => {
    ;(prismaClient.prisma.notification.deleteMany as jest.Mock).mockResolvedValueOnce({})
    ;(prismaClient.prisma.benefit.delete as jest.Mock).mockResolvedValueOnce({})
    const req = new Request('http://localhost', { method: 'DELETE' } as any) as any
