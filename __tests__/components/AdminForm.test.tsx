...
describe("AdminForm", () => {
  it("renders brand form and submits controlled values", async () => {
    ...
    render(<AdminForm type="brand" onSave={onSave} onCancel={onCancel} />);

    fireEvent.change(screen.getByLabelText("Name"), {
      target: { value: "Test Brand" },
    });
    fireEvent.change(screen.getByLabelText("Logo URL"), {
      target: { value: "/images/brands/test.png" },
    });
  ...
  it("renders benefit form and toggles checkbox/number inputs", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Title"), {
      target: { value: "Free Coffee" },
    });
    fireEvent.change(screen.getByLabelText("Description"), {
      target: { value: "Get a free coffee" },
    });
    fireEvent.change(screen.getByLabelText("Redemption Method"), {
      target: { value: "Show app" },
    });
    fireEvent.change(screen.getByLabelText("Validity Type"), {
      target: { value: "birthday" },
    });
    fireEvent.change(screen.getByLabelText("Validity Duration (days)"), {
      target: { value: "30" },
    });
