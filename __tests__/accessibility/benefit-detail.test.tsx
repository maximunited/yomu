...
describe('BenefitDetailPage Accessibility', () => {
  ...
  it('should have proper button labels and ARIA attributes', async () => {
    ...
    const brandWebsiteButton = screen.getByRole('button', { name: /אתר המותג הרשמי/i });
    expect(brandWebsiteButton).toBeInTheDocument();
    expect(brandWebsiteButton).toHaveAttribute('title', 'לחץ לביקור באתר הרשמי של המותג');
    
    const reportButton = screen.getByRole('button', { name: /דווח על מידע שגוי או חסר/i });
