...
describe('Dashboard membership summary count', () => {
  beforeEach(() => {
    global.fetch = jest.fn(async (url: RequestInfo) => {
      ...
      if (u.includes('/api/user/memberships')) {
        ...
        return new Response(JSON.stringify({ memberships: [] }), { status: 200 }) as any
      }
      if (u.includes('/api/benefits')) {
        ...
        const benefits = Array.from({ length: 24 }).map((_, i) => ({
          ...
          brand: { id: brands[i % brands.length], name: `Brand ${brands[i % brands.length]}`, logoUrl: '', website: '', description: '', category: 'food' },
          validityType: 'birthday_entire_month',
          redemptionMethod: 'in-store',
          isFree: true,
        }))
        return new Response(JSON.stringify({ benefits }), { status: 200 }) as any
      }
  ...
  it('falls back to available brand count when memberships are 0', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Active Memberships|חברים פעילים/i)).toBeInTheDocument())
    // The count element is the next div with purple text; we assert it eventually reflects non-zero
    const countEl = await screen.findByText((_, node) => node?.textContent === node?.textContent && node?.className?.includes('text-purple-600'))
