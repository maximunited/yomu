...
describe('languages utilities', () => {
  ...
  it('available languages equals supported entries', () => {
    expect(getAvailableLanguages().length).toBe(Object.keys(SUPPORTED_LANGUAGES).length)
  })
  ...
  it('detectUserLanguage prefers saved language then browser', () => {
    ...
    Object.defineProperty(window, 'navigator', { value: { ...originalNavigator, language: 'ru-RU', languages: ['ru-RU'] } })
    expect(detectUserLanguage()).toBe('ru')

    // unsupported falls back to default
    Object.defineProperty(window, 'navigator', { value: { ...originalNavigator, language: 'xx-XX', languages: ['xx-XX'] } })
